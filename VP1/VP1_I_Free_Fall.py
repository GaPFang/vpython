...
height = 15.0     # ball center initial height

scene = canvas(width = 800, height = 800, center = vec(0, height/2, 0), background = vec(0, 0.5, 0))   # open a window
floor = box(length = 30, height = 0.01, width = 10, color = color.blue)
ball = sphere(radius = size, color = vec(1, 0, 0), make_trail = True, trail_radius = 0.05)
...
while ball.pos.y >= size:                # until the ball hit the ground
    rate(1000)                                  # run 1000 times per second
