...
class CO_molecule:
    ...
    def r_K(self):
        return 1 / 2 * self.C.m * mag2(cross(self.C.v - self.com_v(), norm(self.bond.axis))) + 1 / 2 * self.O.m * mag2(cross(self.O.v - self.com_v(), norm(self.bond.axis)))
    
    def com_K(self):
        return 1 / 2 * ((self.C.m + self.O.m) * mag2(self.com_v()))
