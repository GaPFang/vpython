...
#find collisions between the atoms and the walls, and handle their elastic collisions
    for i in range(N):
        if abs(p_a[i][0]) >= container.length / 2 - size and p_a[i][0]*v_a[i][0] > 0 :
            v_a[i][0] = - v_a[i][0] - 2 * v_W * (p_a[i][0] / abs(p_a[i][0]))
    ...
    if t >= 3E-10:
        Ek = 0
        for i in range(N): Ek += 1 / 2 * m * (v_a[i][0] * v_a[i][0] + v_a[i][1] * v_a[i][1] + v_a[i][2] * v_a[i][2])
        T = Ek / (3 * N * k / 2)
        p = P / (2 * (container.length * container.height + container.height * container.width + container.width * container.length)) / t
        V = container.length * container.height * container.width
        t = 0
        print("stage", stage, "  T=", "{:.2f}".format(T),"  p=", "{:.2e}".format(p),"  V=", "{:.2e}".format(V),"  pV=", "{:.2e}".format(p * V),"  NkT=", "{:.2e}".format(N * k * T),"  pV^gamma=", "{:.2e}".format(p * V ** (5/3)))
        P = 0
