...
# the average velocity and two axis in velocity space
...
while True:
    ...
    for i in range(N):
        if (random() < prob):
            ...
            phi = random() * np.pi
            speed = mag(a_to_v(v_array[i]))
            x, y, z = speed * np.cos(theta) * np.sin(phi), speed * np.sin(theta) * np.sin(phi), speed * np.cos(phi)
            v_array[i] = np.transpose([x, y, z])
